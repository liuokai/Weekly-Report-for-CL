    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        <div className="flex shrink-0 items-center justify-between rounded-t-xl border-b border-gray-100 bg-[#a40035]/5 px-6 py-4">
          <div>
            <h2 className="text-lg font-bold text-[#a40035]">新店经营情况明细（费用预算）</h2>
...
const RampUpStoreOperationStatusContainer = () => {
  ...
  const latestData = React.useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      if (!latestByStore[code] || (item.month || '') > (latestByStore[code].month || '')) {
        latestByStore[code] = item;
      }
    });
  ...
  const { sortedData, sortConfig, handleSort } = useTableSorting(columns, latestData, {
    key: 'city_name',
    direction: 'asc',
  });
            ...
            {loading ? (
              ...
              pagedData.map((item, index) => {
                ...
                return (
                  ...
                  <td className={`${TABLE_BODY_CELL_CLASS} text-center`}>{formatCurrency(item.group_buy_discount_actual)}</td>
                  <td className={`${TABLE_BODY_CELL_CLASS} text-center`}>{formatCurrency(item.offline_ad_fee_actual)}</td>
                  <td className={`${TABLE_BODY_CELL_CLASS} text-center`}>{formatCurrency(item.new_guest_discount_actual)}</td>
                  <td className={`${TABLE_BODY_CELL_CLASS} text-center`}>{formatCurrency(item.exhibition_fee_actual)}</td>
                  <td className={`${TABLE_BODY_CELL_CLASS} text-center`}>{formatCurrency(item.masseur_commission_actual)}</td>
