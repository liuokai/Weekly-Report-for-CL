...
SELECT dc.statistics_city_name                   AS city_name,                                                                                           -- '城市名称'
       ...
       max(a.opening_date)                       AS opening_date,                                                                                        -- '开业日期'
       max(b.ramp_up_period)                     AS ramp_up_period,                                                                                      -- '爬坡期长度'
       nvl(max(b.ramp_up_month_count), '已结束') AS ramp_up_month_count,                                                                                 -- '当前爬坡期月数'
       max(DATE_FORMAT(DATE_ADD(b.opening_date, INTERVAL b.ramp_up_period-1 + IF(DAY(b.opening_date) > 15, 1, 0) MONTH), '%Y-%m')) AS ramp_up_end_month ,-- 爬坡期结束月,
...
FROM data_warehouse.dws_profit_store_detail_monthly a
         LEFT JOIN data_warehouse.dm_city dc ON a.city_code = dc.city_code
         LEFT JOIN data_warehouse.dws_new_store_commission_monthly b ON a.store_code = b.store_code and b.month = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m')
