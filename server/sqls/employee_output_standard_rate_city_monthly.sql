...
WITH monthly_city_output_metrics AS (
    ...
    GROUP BY statistics_city_code, month, statistics_city_name),
     rate_calculation AS (
         -- 第二步：计算月度、城市维度的推拿师产值达标率
         SELECT statistics_city_name,
                month,
                total_massagists,
                standard_count,
                ROUND(
                        standard_count / NULLIF(total_massagists, 0) * 100, 2
                ) AS output_standard_rate
         FROM monthly_city_output_metrics)

-- 第三步：计算同城、同月的同比（2025 vs 2024）
SELECT curr.statistics_city_name AS statistic_city_name,
...
ORDER BY curr.statistics_city_name,
         curr.month;