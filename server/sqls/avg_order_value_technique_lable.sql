...
select technique_name                                                                                          as '技法名称',
       ...
       round(sum(order_actual_payment) / sum(sum(order_actual_payment)) over (partition by technique_name),
             6)                                                                                                as '技法营业额占比',
       round(sum(order_actual_payment) / sum(sum(order_actual_payment)) over (partition by 1),
             6)                                                                                                as '总营业额占比'
from order_detail
group by technique_name, label_name
order by technique_name, label_name;
