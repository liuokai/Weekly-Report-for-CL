...
WITH monthly_aggregate AS (
    -- 第一步：按月汇总全公司基础财务指标
    SELECT month                     AS stat_month,    -- 月份 (YYYY-MM)
           SUM(main_business_income) AS total_revenue, -- 营业额汇总
           SUM(net_profit)           AS total_profit,  -- 利润额汇总
           SUM(nvl(labor_cost, 0)) + sum(nvl(fixed_cost, 0)) + sum(nvl(variable_cost, 0)) + sum(nvl(income_tax, 0)) +
           sum(nvl(service_fee, 0))  AS total_cost     -- 总成本              
