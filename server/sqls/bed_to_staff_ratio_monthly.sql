...
WITH monthly_base AS (SELECT a.store_feature_record_time,
...
-- 自连接实现同比：月相等、年=年+1
SELECT curr.s_year                                                           AS stat_year,--  年
       curr.s_month                                                          AS stat_month,-- 月
       curr.store_feature_record_time                                        AS stat_month_date,-- 年月
       prev.store_feature_record_time                                        AS last_stat_month_date,-- 上年同期年月
       curr.total_staff_on_duty, --  在职人数
       curr.total_bed_count,-- 床位数
       curr.overall_staff_bed_ratio                                          AS current_month_ratio,-- 在职床位配置比
       prev.overall_staff_bed_ratio                                          AS last_year_same_month_ratio,-- 上年 在职床位配置比
       ROUND(curr.overall_staff_bed_ratio - prev.overall_staff_bed_ratio, 2) AS yoy_diff_value,-- 差异值
