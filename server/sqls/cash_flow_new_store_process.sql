...
SELECT
    ...
    sum_new_target AS new_store_target, -- 原字段：新店目标
    sum_new_num    AS new_store_count, -- 原字段：新店数量
    ...
    END AS new_store_target_status, -- 原字段：新店目标完成情况

    sum_reinstall_target AS reinstall_target, -- 原字段：重装目标
    sum_reinstall_num    AS reinstall_count, -- 原字段：重装数量
...
FROM (
    SELECT
        ...
        GROUPING(city_name) AS is_total_row_flag, -- 原字段：is_total_row
...
ORDER BY
    month ASC,
    is_total_row_flag ASC,      -- 0 是明细，1 是合计，确保合计在月度末尾
