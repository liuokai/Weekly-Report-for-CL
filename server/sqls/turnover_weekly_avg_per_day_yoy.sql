...
WITH weekly_base AS (
    ...
    SELECT
        ...
        -- ✅ 按 ISO 周口径：year 从 week_start 派生
        YEAR(
            STR_TO_DATE(CONCAT(YEARWEEK(off_clock_time, 1), ' Monday'), '%x%v %W')
        ) AS sales_year,

        -- ✅ 按 ISO 周口径：week 从 week_start 派生
        WEEK(
            STR_TO_DATE(CONCAT(YEARWEEK(off_clock_time, 1), ' Monday'), '%x%v %W'),
            1
        ) AS sales_week,
...
weekly_avg AS (
    ...
    SELECT *,
           IF(active_days = 0, 0,
              ROUND(weekly_total_revenue / active_days, 2)
           ) AS daily_avg_revenue
    FROM weekly_base
)
...
SELECT
    ...
    IF(
        prev.daily_avg_revenue IS NULL OR prev.daily_avg_revenue = 0,
        NULL,
        ROUND(
            (curr.daily_avg_revenue - prev.daily_avg_revenue)
            / prev.daily_avg_revenue * 100,
            2
        )
    ) AS `yoy_change`
