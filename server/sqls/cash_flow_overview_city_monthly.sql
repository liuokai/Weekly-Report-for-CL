-- 实际成本占比情况
SELECT statistics_city_name AS '城市',
       -- 总部提取管理费
       ...
       -- 变动成本 - 资产维护
       CASE WHEN SUM(NVL(main_business_income, 0)) = 0 THEN 0 ELSE ROUND(SUM(NVL(consumables_purchase_fee, 0) + NVL(linen_purchase_fee, 0) + NVL(washing_fee, 0) + NVL(asset_maintenance_fee, 0)) / SUM(NVL(main_business_income, 0)), 6) END AS '变动成本 - 资产维护',
